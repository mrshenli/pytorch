...
TensorPipeAgent::TensorPipeAgent(
...

  {
    ...
    if (opts_.devices.empty()) {
      ...
      // no duplications are allowed in opts_.devices
      TORCH_CHECK(
          opts_.devices.size() == optsDeviceSet.size(),
          "Detected duplication in TensorPipeRpcBackendOptions devices field.");

      // opts_.devices must be a superset of local devices in device_maps
      std::vector<c10::DeviceIndex> cut;
      std::set_difference(
          deviceSet.begin(), deviceSet.end(),
          optsDeviceSet.begin(), optsDeviceSet.end(),
      ...
      if (!cut.empty()) {
        ...
        TORCH_CHECK(
            ...
            "local devices ",
            oss.str(),
            "are used in device_maps but not included the devices field. ");
      }
      */
