...
Message BuiltinOp::toMessage() {
  ...
  // need this as somehow we cannot add multiple IValues to the pickler
  // the code below prevents getting a message twice from a BuiltinOp
  stack_.emplace_back(toString(op_->schema()));

  pickler.start();
  pickler.addIValue(std::move(stack_));
  pickler.finish();
