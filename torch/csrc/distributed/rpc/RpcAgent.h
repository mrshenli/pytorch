...
class RpcAgent {
 public:
  ...
  // Send a message to the worker with name ``to`` and returns a Future.
  // If the message type if either BUILTIN_OP or PYTHON_UDF_OP, the Future will
  // be complete when the response arrives. For other message types, the Future
  ...
  virtual std::shared_ptr<Future> send(std::string to, Message message) = 0;
