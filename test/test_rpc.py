...
class RpcTest(TestCase):
    ...
    def _init_rpc(cls, rank, filename, world_size):
        print ("=== init rpc for rank ", rank, ", world_size ", world_size)
        name_dict = {'worker%d' % r : r for r in range(world_size)}
        store = dist.FileStore(filename, world_size)
        dist.init_process_group(
            backend='gloo', rank=rank, world_size=world_size, store=store)
        name_store = dist.MapNameStore(name_dict)
        dist.init_rpc('worker' + rank, name_store=name_store)
    ...
    def _test_builtin(cls, rank, filename, c2p, p2c):
        ...
        """
        print("== before invoking rpc")
        if rank == 0:
            ret = dist.invoke_rpc('worker1', 'aten::add', torch.ones(2, 2), torch.ones(2, 2))
            c2p.put((rank, torch.ones(2, 2) * 2, ret))
        else:
            ret = dist.invoke_rpc('worker0', 'aten::mul', torch.ones(3, 3), torch.zeros(3, 3))
            c2p.put((rank, torch.zeros(2, 2), ret))
        p2c.get()
        """
        pass
    ...
    def test_builtin(self):
        self._test_multiprocess(RpcTest._test_builtin, 0)
